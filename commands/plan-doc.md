...
- `<task-slug>`: required unless interactive; a stable task name without a date
  unless needed for disambiguation. Follow the host project's documentation
  naming rules and primary documentation language. Use lowercase kebab-case
  only when the host project does not define a stricter naming convention.
...
plan`, treat the marker as an explicit confirmation of the default and do not
