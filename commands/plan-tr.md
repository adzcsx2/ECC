...
if [ -f "$GITIGNORE" ] && grep -qxE "[[:space:]]*${IGNORE_ENTRY}[[:space:]]*" "$GITIGNORE"; then
  ...
  printf '\n# ECC plan-tr 隔离 worktree（本地临时，禁止提交/推送）\n%s\n' "$IGNORE_ENTRY" >> "$GITIGNORE"
...
prompt: "在以下 worktree 目录中执行严格 TDD: <WORKTREE_PATH>
         ...
         完成所有计划项后执行 git add -A && git commit（至少一次），commit message 必须使用中文。
         所有代码注释（行注释 //、块注释 /* */、文档注释 /// /** */ 等）必须使用中文编写，禁止英文注释。
