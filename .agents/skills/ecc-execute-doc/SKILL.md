---
name: ecc-execute-doc
description: Execute a plan-doc execution document phase by phase with same-model delegation, main-agent quality gates, retries, and automatic progress advancement.
metadata:
  origin: user-migrated
  source: commands/execute-doc.md
---

# ECC execute-doc（Codex 原生入口）

在 Codex 中调用 `$ecc-execute-doc`，并提供 `00-执行文档.md` 的文件或目录路径。
此 skill 是 `commands/execute-doc.md` 的原生 Codex 版本；同仓库的 `commands/`
文件仍作为 Claude Code 与其他兼容 harness 的入口保留。
...
4. **context 接近上限（主代理判定已无法安全承载下一 phase）**：主代理**自主**完成当前 phase（含提交、把指针固化到下一未完成 phase 的起点）后，输出 **Resume 续接指令**并停止。停止动作**禁止**包含任何疑问句（「是否继续」「要不要」「先到此作为交付点」一律禁止），只输出祈使句式续接指令，例如：「当前会话 context 已满，Phase N 已固化。请新开会话执行：『请阅读 <DOC_PATH>，执行其 Resume 协议，从 Phase N 继续。』」——这是**主代理主导的换会话**，不是把决策抛给用户

**禁止**在任何 phase 边界或审计边界说「是否继续」「请确认」「先到此作为交付点」或等待用户确认。除上面四种情况外，遇到任何需要决策的小问题，按本命令既定规则自行处理后继续。
...
> **2e 是停车高发点，就地焊死**：`phase += 1` 之后**立即**执行 2a 的前置验证（progress pointer 复核 / git 状态记录），**禁止**在 2e 与 2a 之间插入任何面向用户的话术——包括但不限于「Phase N 已完成，进入 Phase N+1」「已完成 X 个 phase，剩余 Y 个」这类阶段性总结。更新指针、追加 execution log 都是过程记录，做完即继续。若你正准备输出一段「进度汇报」——这恰恰说明下一步已明确，**必须直接执行 2a**，不得停下。
...
- **`$ecc-plan-doc`** —— 生成 `00-执行文档.md`，是本 skill 的**输入**。plan-doc 描述「execution companion」时应指向本 skill 而非 `/ecc:plan`
...
- 每个 phase 完成后停下问「是否继续下一个 phase」（违反自动推进）——**最高频、最严重**。字面反例（出现即判定违规）：「指针已指向 P2.1，需要我继续 Phase 2 吗？」「先到此作为 Phase 1 交付点」「单会话 context 无法承载，是否换会话继续」。任何 phase 边界的疑问句 / 伪交付话术都命中本条。context 压力走封闭清单第 4 条的**主代理自主 Resume**，而非停车询问
- 把「更新进度 / 追加日志」当作停顿交付物
