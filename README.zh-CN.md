...
**完成！** 你现在可以使用 63 个代理、249 个技能和 85 个命令。
...
| `/ecc:plan-doc` | 生成模块化任务文档，并执行产品对齐与鲁棒性修订门禁 | [README](commands/plan-doc/README.md) |
...
| `/ecc:prp-prd` | 生成经过调研的产品需求文档；优先使用 `docs/product/` 和中文文件名，无 `docs/` 时使用旧路径 | [README](commands/prp-prd/README.md) |
...
| `/ecc-plan-doc`        | 多会话、多阶段工作 | 持久化模块化执行文档并强制产品对齐质量门禁 |
...
检测到 Codex 后，`install.sh` 和 `install.ps1` 会自动把顶层 `commands/*.md` 同步到 `~/.agents/skills/`，生成 `ecc-*` skill wrapper。Codex 中使用与 Claude 命令对应的技能：
...
每次安装都会刷新这些生成文件，并清理已废弃的生成型 `ecc-*` skill；用户自己写的文件不会被删除。如果偏好 `/prompts:ecc-*` alias，可设置 `ECC_SYNC_CODEX_PROMPTS=1` 额外生成 prompt alias。
...
- **Codex 命令同步**：如果存在 `~/.codex` 或 `CODEX_HOME`，安装时默认刷新 `~/.agents/skills/` 下的 Codex skill wrapper。设置 `ECC_SYNC_CODEX_PROMPTS=1` 可额外生成 `~/.codex/prompts/` 下的 prompt alias。显式执行 `./install.sh --target codex` 会安装 Codex baseline 并同步同一批技能。
